...
#include "neural_network_letters.h"
...
void create_neural_network(layer_t * neural_network, int * topology, int topology_size){
    int i;
...
/*
Derivada parcial de la funcion RELU
*/
...
void regression_function (float * X, layer_t * layer, float * output){
    for(int i = 0; i < layer->neurons; i++ ){
        ...
        for(int j = 0; j < layer->connections; j++ ){
            if(abs(output[i]) > 10000){
                output[i] = 0.0;
            }
            output[i] += PIXEL_SCALE_WHITE_255(X[j]) * layer->W[i][j];                
        }
    }
}
...
void cleanArrayInteger(int * array, int length){
    for(int i = 0; i < length; i++){
        array[i] = 0;
    }
}
...
void start(int count_img, char * datasetType) { 
    printf("%d %s \n",count_img,datasetType);
    float learning_rate = 0.5f;
    int topology[] = {IMAGE_SIZE, 7, 8};  
    int getImageTest = 0;
    layer_t neural_network[LAYERS];
    image_t * images_set = malloc(count_img * sizeof(image_t)); 

    list_directory_files(count_img, images_set, datasetType);
    create_neural_network(neural_network, topology, LAYERS);
...
void compute_image(char * datasetType) {
    printf("%s \n",datasetType);
    layer_t neural_network[LAYERS];
    image_t images_set; 
    int count_img = 1;
    int getImageTest = 1;
    list_directory_files_single_img(count_img, &images_set, datasetType);
    deserialize(*neural_network);
    neural_network_training_step(neural_network, images_set.pixels, images_set.label, 0, 0);
}
